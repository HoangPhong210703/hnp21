...
export function getPostPathSegments(
  ...
  const isIndexFile = /\/index\.mdx?$/.test(filePath);
  ...
  return filePath
    ...
    // Drop the filename; for folder posts (index.md/index.mdx) also drop the
    // post's own folder, since that folder name is already used as the slug.
    .slice(0, isIndexFile ? -2 : -1)
    .map(slugify);
}
